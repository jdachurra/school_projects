...
const int BOARD_SIZE_DEF = 6;
const int NUMBER_OF_SYMBOLS_DEF = BOARD_SIZE_DEF / 2 ;
...
public:
    ...
    GameBoard(uint board_size_temp, uint number_of_symbols_temp);
    ...
    ///added QLabel* label_error
    bool fill_randomly(unsigned int seed, QLabel* label_error);
    ...
    // otherwise returns false.
    ///added QLabel* label_error
    bool fill_from_input(const std::string& input, QLabel* label_error);
    ...
    ///getter for the board_
    std::vector<std::vector<Element_type>> getBoardVector() const;

    // Tulostaa pelilaudan.
    // Prints the gameboard.
    void print() const;

    unsigned int board_size; //defined in gameboard.cpp, line 5
